...
static void print_mode(char *str, Camera_Mode mode)
{
    switch (mode) {
        case CAMERA_MOVE:
            DEBUG_PRINT("%s MOVEn", str);
            break;
        default:
            DEBUG_PRINT("%s FIXn", str);
            break;
    }
}
...
{
    if (Game_Status_find_character(access, result, point) == FOUND) {
        return YES;
    } else {
        return NO;
    }
...
camera_horizon_mode_setup(Camera_Access access, Point point, int32_t x)
{
    ...
    int32_t map_end_x = Map_Type_bottom_right_x(map);
...
camera_vertical_mode_setup(Camera_Access access, Point point, int32_t y)
{
    ...
    Map_Access map = access->map;
    int32_t map_end_y = Map_Type_bottom_right_y(map);
 ...
 * @return Camera 物件
 *
 * MAX_X 的預設值為 25，MAX_Y 的預設值為 21，可透過 Graphic_Camera_set_max_x 跟 Graphic_Camera_set_max_y
 * 來修改。
*/
Camera_Access Graphic_Camera_start(void)
{
    ...
    Graphic_Camera_set_max(access,.x = MAX_X,.y = MAX_Y);
 ...
 * @param self 要釋放的角色物件
*/
void Graphic_Camera_stop(Camera_Access self)
{
    RECT(free) (rectangle);
 ...
 * @param other 要設定的數值
*/
void Graphic_Camera_set_max_by_point(Camera_Access self, Point other)
{
    int x = other->x;
    int y = other->y;
...
*/
void Graphic_Camera_set_player(Camera_Access self, Character_Access player)
{
 ...
 * @param dead 指定的圖形
*/
void Graphic_Camera_set_dead_style(Camera_Access self, Style_Access dead)
{
    self->dead = dead;
}
 ...
 * @param map 指定的地圖物件
*/
void Graphic_Camera_set_map(Camera_Access self, Map_Access map)
{
    Map_Type_move_bottom_right(map, -1, -1);
    self->map = map;
}
...
*/
bool
Graphic_Camera_take(Camera_Access self,
                    ...
                    Character_Access current, Message_Type message)
{
    ...
    if (result == NO) {
        ...
        char *attack_message;
        {
            int len = snprintf(NULL, 0,
                               ...
                               Status_get_relation_string(current->status),
                               npc->status->name->str,
                               Status_get_relation_string(npc->status));
        ...
        switch (npc_relation) {
            case FACTION_PLAYER:
                is_alive =
                    Game_Status_attack_player_by(from_pool, current);
                break;
            case FACTION_ALLY:
                is_alive =
                    Game_Status_attack_ally_by(from_pool, current, npc);
                break;
            case FACTION_ENEMY:
                is_alive =
                    Game_Status_attack_enemy_by(from_pool, current, npc);
                break;
            case FACTION_NEUTRAL:
                is_alive =
                    Game_Status_attack_neutral_by(from_pool, current, npc);
                break;
            default:
                break;
        }
    ...
    Game_Status_calculate_graph_position(from_pool, rectangle);
  DONE:
    return true;
}
