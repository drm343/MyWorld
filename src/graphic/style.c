...
*/
void Style_Pool_stop(Style_Pool_Access self)
{
    uint8_t counter = 0;
    Style_Access item = NULL;
    while (item = Style_Pool_next(self, &counter)) {
...
*/
void Style_Pool_free_texture(Style_Pool_Access self)
{
...
*/
Style_Access Style_Pool_malloc(Style_Pool_Access self)
{
...
*/
Style_Access Style_Pool_find(Style_Pool_Access pool_access,
                             const char *name)
{
    uint8_t used = pool_access->max_size - pool_access->current_size;
    Style_Access result = NULL;
    ImmutableString find_name = String_create(name);

    for (uint8_t count = 0; count < used; count++) {
        result = &(pool_access->pool[count]);

        if (String_equal(result->name, find_name) == YES) {
            goto DONE;
        }
    }
    result = NULL;
...
*/
Style_Access Style_Pool_next(Style_Pool_Access self,
...
*/
void Style_Pool_debug(Style_Pool_Access self)
{
#ifdef DEBUG
    uint8_t used = self->max_size - self->current_size;
    Style_Access result = NULL;

    DEBUG_PRINT("style is null? %sn", BOOL_STRING(result));
    for (uint8_t count = 0; count < used; count++) {
        result = &(self->pool[count]);
        DEBUG_PRINT("%s : %s : %sn", result->name->str, result->mark->str,
