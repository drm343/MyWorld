                                ...
                                const char *name, Map_Access map)
{
    ...
    if (result == FOUND) {
        ...
        Status_copy(npc->status, origin_npc->status);

        Status_set_name(npc->status, name);
        ...
        Point bottom_right = Map_Type_bottom_right(map);
        Character_set_random_position(npc, Point_x(bottom_right),
                                 ...
                                 Faction_List enemy_group)
{
    ...
    Point target_position = Character_get_position(target);
    Point self_position = Character_get_position(self);
                                         ...
                                         Faction_List follow_faction)
{
    ...
    Point target_position = Character_get_position(player);
    Point self_position = Character_get_position(self);
...
static bool pool_all_copy(Status_Pool * from, Status_Pool * to)
{
    ...
    for (uint8_t count = 0; count < used; count++) {
        ...
        if (from_item->is_used == IN_USE) {
            ...
            Status_copy(to_status, from_status);
        }
    }
    return false;
}
                                          ...
                                          Point point)
{
    for (*npc = list->reset_iterator(list); *npc != NULL;
         *npc = list->next(list, *npc)) {
        Point npc_position = Character_get_position(*npc);
...
*/
Game_Status_Access
Game_Status_create(uint8_t max_config_size, uint8_t max_instance_size)
{
...
*/
void Game_Status_free(Game_Status * self)
{
    /*
...
*/
Execute_Result
Game_Status_parse_npc_config(Game_Status * self,
                             ...
                             Style_Pool_Access style_pool)
{
    ...
    if (setting != NULL) {
        ...
        for (counter = 0; counter < total_counter; counter++) {
            ...
            Style_Access style = Style_Pool_find(style_pool, race);

            Character_Access npc = Character_Factory_malloc(self->prepare);
            Status_set_name(npc->status, name);
            Status_set_race(npc->status, race);
            Character_set_style(npc, style);
...
*/
Found_Result
Game_Status_find_character(Game_Status * self,
 ...
 */
void
Game_Status_calculate_graph_position(Game_Status * self,
                                     Rectangle rectangle)
{
 ...
 */
Character_Access
Game_Status_use_ally(Game_Status * self, const char *race,
                     const char *name, Map_Access map)
{
    Character_Access npc = use_npc(self, race, name, map);

    if (npc != NULL) {
        Status_set_ally(npc->status);
        add_ally(self, npc);
    }
    return npc;
}
 ...
 */
Character_Access
Game_Status_use_enemy(Game_Status * self, const char *race,
                      const char *name, Map_Access map)
{
    Character_Access npc = use_npc(self, race, name, map);

    if (npc != NULL) {
        Status_set_enemy(npc->status);
        add_enemy(self, npc);
    }
    return npc;
}
 ...
 */
Character_Access
Game_Status_use_neutral(Game_Status * self,
                        const char *race, const char *name, Map_Access map)
{
    Character_Access npc = use_npc(self, race, name, map);

    if (npc != NULL) {
        Status_set_neutral(npc->status);
 ...
 */
Character_Access Game_Status_use_player(Game_Status * self)
{
    Character_Access player = Character_Factory_malloc(self->used_pool);
    Status_init(player->status);
...
*/
Message_Type
Game_Status_action(Game_Status * self, Character_Access current_character)
{
    Message_Type result = DO_NOTHING;
...
*/
Is_Alive
Game_Status_attack_enemy_by(Game_Status * self,
                            Character_Access current,
                            Character_Access target)
{
    Is_Alive result = Status_attack(current->status, target->status);
...
*/
Is_Alive
Game_Status_attack_ally_by(Game_Status * self,
                           Character_Access current,
                           Character_Access target)
{
    Is_Alive result = Status_attack(current->status, target->status);
...
*/
Is_Alive
Game_Status_attack_neutral_by(Game_Status * self,
                              Character_Access current,
                              Character_Access target)
{
    Is_Alive result = Status_attack(current->status, target->status);
 ...
 * @return 目標的生存狀況
*/
Is_Alive Game_Status_attack_player_by(Game_Status * self,
                                      Character_Access current)
{
 ...
 * @return 第一個角色
*/
Character_Access Game_Status_reset_iterator(Game_Status * self)
{
    Character_Access npc = NULL;
 ...
 * @return 下一個角色
*/
Character_Access Game_Status_next(Game_Status * self, Character_Access npc)
{
 ...
 * @return 角色行動順序
*/
Iterator Game_Status_action_order(Game_Status * self)
{
