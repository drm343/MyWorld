 ...
 * 請確保傳入的物件不是 NULL
 */
void Character_init(Character_Access self)
{
 ...
 */
void Character_free(Character_Access self)
{
 ...
 */
void Character_copy(Character_Access self, Character_Access from)
{
    Status_copy(self->status, from->status);
    self->Mark = from->Mark;
}
 ...
 * @param self 要顯示的角色
 */
void Character_print_status(Character_Access self)
{
    Status_print_status(self->status);
    DEBUG_PRINT("Mark: %sn", self->Mark->mark->str);
}
#endif
 ...
 * @param style 角色圖形
 */
void Character_set_style(Character_Access self, Style_Access style)
{
    self->Mark = style;
}
 ...
 */
void
Character_set_random_position(Character_Access self,
 ...
 * @return 回傳座標
 */
Point Character_get_position(Character_Access self)
{
