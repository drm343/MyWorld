package body My_World.Sub_Window is
   ...
   begin
      ...
      Sub_Window.Window := NCurses.Derived_Window
        (Main_Window,
         NCurses.Line_Position (Lines - 2),
         NCurses.Column_Position (Columns - 2),
         NCurses.Line_Position (Start_Line + 1),
         NCurses.Column_Position (Start_Column + 1));

      NCurses.Box (Window);
      NCurses.Refresh (Window);
      NCurses.Delete (Window);
   ...
   begin
      NCurses.Get_Cursor_Position (Sub_Window.Window,
                                   Useless_Line,
                                   Target_Column);
   ...
   is
      ...
      Target_Column : NCurses.Column_Position := NCurses.Column_Position (Column);
   begin
      for counter in Integer range 0 .. Max_Lines loop
         NCurses.Add (Sub_Window.Window,
   ...
   begin
      if Position.X >= 0
        and Position.X < Sub_Window.Columns
        and Position.Y >= 0
        and Position.Y < Sub_Window.Lines then
   ...
   begin
      ...
      Max_Position := Position.X + Position.Y * Integer (Columns) + Integer (Message'Length);

      if Position.X >= 0
        and Position.X < Sub_Window.Columns
        and Position.Y >= 0
        and Position.Y < Sub_Window.Lines
        and Max_Position <= Sub_Window.Max_Size then
   ...
   procedure Clean (Sub_Window : in out Sub_Window_Type) is
   begin
   ...
   begin
      NCurses.Move_Cursor (Sub_Window.Window,
