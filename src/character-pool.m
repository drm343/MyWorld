...
@implementation Character_Pool + (id) create:(uint8_t)
max_config_size with_instance_size:(uint8_t) max_instance_size
{
...
-(Execute_Result) parse_npc_config:(const char *)
file_path with_style:(Style_Pool_Access) style_pool
{
...
-(Status_Access) sign_in {
  ...
  */
-(Found_Result) find_character:(Status_Access *)
npc with_position:(Point_Access) point
{
...
-(Status_Access) use_ally:(const char *)
race with_name:(const char *)
name and_map:(Map_Access) map
{
...
-(Status_Access) use_enemy:(const char *)
race with_name:(const char *)
name and_map:(Map_Access) map
{
  Status_Access npc =[self use_npc: race with_name: name and_map:map];
...
-(Status_Access) use_neutral:(const char *)
race with_name:(const char *)
name and_map:(Map_Access) map
{
  Status_Access npc =[self use_npc: race with_name: name and_map:map];
...
-(Status_Access) use_npc:(const char *)
race with_name:(const char *)
name and_map:(Map_Access) map
{
...
-(Message_Type) action:(Status_Access) current_character {
   ...
   * 場，因此必須透過 pool 轉呼叫。
  */
-(Is_Alive) attack_enemy_by:(Status_Access)
current with_target:(Status_Access) target
{
   ...
   * 場，因此必須透過 pool 轉呼叫。
  */
-(Is_Alive) attack_ally_by:(Status_Access)
current with_target:(Status_Access) target
{
    Is_Alive result = character.attack(current, target);
   ...
   * 場，因此必須透過 pool 轉呼叫。
  */
-(Is_Alive) attack_neutral_by:(Status_Access)
current with_target:(Status_Access) target
{
    Is_Alive result = character.attack(current, target);
